...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      copy(countDown-1,Motion.movePoint(position,direction,Settings.propulsionSpeed))
    else
...
case class Particle(horizontalSpeed: Double, verticalSpeed: Double, position: Point, color: Color):
  ...
  def next: Particle =
    ...
    copy(horizontalSpeed= updatedHorizontalSpeed,verticalSpeed= updatedVerticalSpeed, position = updatedPosition,color = color)
